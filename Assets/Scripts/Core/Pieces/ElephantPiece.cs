...
public class ElephantAttackStrategy : IAttackable
{
    ...
    /// Слон перемещается только на конечную клетку (target).
    /// </summary>
    public void ExecuteAttack(Piece piece, Vector3Int target, IBoardManager boardManager, bool isRangedAttack)
    {
        ...
        {
            // Если две цели, атакуем первую без перемещения, вторую с перемещением
            piece.SelectAttack(targets[0], isRangedAttack, moveToTarget: false);
            // Рассчитываем длительность анимации для первой цели
